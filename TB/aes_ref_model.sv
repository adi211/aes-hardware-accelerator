package aes_ref_pkg;
  ...
  function automatic logic [7:0] aes_sbox(input logic [7:0] a);
    aes_sbox = AES_SBOX[int'(a)];
  endfunction

  function automatic logic [127:0] subbytes128(input logic [127:0] in_vec);
    logic [127:0] out_vec;
    for (int i = 0; i < 128; i += 8) begin
      out_vec[i +: 8] = aes_sbox(in_vec[i +: 8]);
    end
    return out_vec;
  endfunction
  ...
  function automatic logic [127:0] mixcolumns128(input logic [127:0] in_vec);
    logic [127:0] out_vec;
    for (int i = 0; i < 4; i++) begin
      out_vec[(i*32) +: 32] = mix_single_column(in_vec[(i*32) +: 32]);
    end
    return out_vec;
  endfunction

  function automatic aes_ref_res_t aes_ref_compute(input logic [127:0] data_in);
    aes_ref_res_t r;
    r.sb  = subbytes128(data_in);
    r.out = mixcolumns128(r.sb);
    return r;
  endfunction
